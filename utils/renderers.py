...
class GithubMarkdownRenderer(MarkdownRenderer):
    """
    Renders the specified markdown content using the GitHub Markdown API.
    """

    def __int__(self, user_content=None, context=None, api_url=DEFAULT_API_URL, raw=None):
        super(GithubMarkdownRenderer, self).__init__(user_content, context)
    ...
    def render(self, text, auth=None):
        ...
        if not isinstance(text, expected):
            raise TypeError('Expected a Unicode string, got {!r}.'.format(text))
...
class OfflineRenderer(MarkdownRenderer):
    pass
